...
class SpikeMultiDimensionalAttention(nn.Module):
    def __init__(self, T, C, reduction_t=4, reduction_c=16, kernel_size=3):
        super().__init__()
        self.T = T
        self.C = C

        # Temporal attention (applies across T dimension)
        self.temporal_fc = nn.Sequential(
            nn.AdaptiveAvgPool3d((1, 1, 1)),  # Pool over (T, H, W)
            layer.Conv3d(1, 1, kernel_size=1, step_mode='m'),  # SpikingJelly Conv3d
            nn.Sigmoid()
        )

        # Channel attention (applies across C)
        self.channel_fc = nn.Sequential(
            nn.AdaptiveAvgPool3d((1, 1, 1)),
            layer.Conv3d(C, C // reduction_c, kernel_size=1, step_mode='m'),
            nn.ReLU(inplace=True),
            layer.Conv3d(C // reduction_c, C, kernel_size=1, step_mode='m'),
            nn.Sigmoid()
        )

        # Spatial attention (applies across H, W)
    ...
    def forward(self, x):
        ...
        out = x_spatial.permute(2, 0, 1, 3, 4).contiguous()  # [T, B, C, H, W]
        return out
...
class MultiDimensionalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # spatial_pool = x_chn.mean(2, keepdim=True)  # [B, 1, T, H, W]
        # spatial_att = self.spatial_conv(spatial_pool)  # [B, 1, T, H, W]
        # x_spatial = x_chn * spatial_att
...
class Spiking_Residual_Block(nn.Module):
    def __init__(self, dim):
        ...
        self.residual = nn.Sequential(
            ...
            ThresholdDependentBatchNorm2d(dim),

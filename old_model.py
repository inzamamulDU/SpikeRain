...
class OLD_Feature_Refinement_Block(nn.Module):  
...
class Feature_Refinement_Block(nn.Module):  
    def __init__(self, channel, reduction=8):
        ...
        # Channel Attention (same as before)
        self.ca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channel, channel // reduction, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
        ...
        # Spatial Attention (using Depthwise Separable Conv)
        self.sa = nn.Sequential(
            nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1, groups=channel, bias=False),  # depthwise
            nn.Conv2d(channel, channel // 8, kernel_size=1, bias=False),  # pointwise
            nn.ReLU(inplace=True),
            nn.Conv2d(channel // 8, channel, kernel_size=1, bias=False),  # back to original channel
            nn.Sigmoid()
        )

        # Learnable gating mechanism for adaptive fusion
        self.gate = nn.Sequential(
            nn.Conv2d(channel * 3, channel, kernel_size=1, bias=True),
            nn.Sigmoid()
        )

    def forward(self, x):
        ca_out = self.ca(x) * x                     # Channel attention applied to input
        sa_out = self.sa(x) * x                     # Spatial attention applied to input

        fusion = torch.cat([x, ca_out, sa_out], dim=1)  # Combine x, CA, SA
        gate = self.gate(fusion)                        # Learn a soft gate

        out = gate * ca_out + (1 - gate) * sa_out + x   # Fusion + residual connection
        return out
...
class ESDNet(nn.Module):
    def __init__(self, inp_channels=3, out_channels=3, dim=24, en_num_blocks=[4, 4, 6, 6], de_num_blocks=[4, 4, 6, 6],
        ...
        self.reduce_chan_level2 = nn.Sequential(
            ...
            ThresholdDependentBatchNorm2d(int(dim * 2 ** 1)),
        )
